...
export const CharacterCard = ({ character }) => {
    ...
    return (<div className={`bg-game-accent p-4 rounded-lg border-l-4 ${character.alive ? 'border-green-500' : 'border-gray-500 opacity-50'}`}>
      ...
      <div className="mb-3">
        ...
        <div className="w-full bg-gray-700 rounded-full h-3">
          <div className={`h-3 rounded-full transition-all ${hpPercentage > 50 ? 'bg-green-500' : hpPercentage > 25 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{ width: `${hpPercentage}%` }}/>
        </div>
      </div>
      ...
      {/* バフ表示 */}
      {character.buffs.length > 0 && (<div className="mt-3 flex flex-wrap gap-1">
          {character.buffs.map((buff, index) => (<span key={index} className="px-2 py-1 bg-purple-600 text-xs rounded">
              {buff.type}
            </span>))}
        </div>)}

      {/* アイテム表示 */}
      {character.items.length > 0 && (<div className="mt-2">
