...
export const MapView = ({ onAreaClick, selectedArea, characters, towers }) => {
    const width = 800;
    const height = 800;
    // エリアの座標定義（LoL風の対角線配置）
    const areas = {
    ...
    return (<div className="bg-gray-900 rounded-lg p-4 border-2 border-gray-700">
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="w-full h-auto" style={{ background: '#0a0e1a' }}>
        {/* 背景グラデーション */}
        <defs>
        ...
        {/* リバー（中央の川） */}
        <path d="M 0 400 Q 200 350, 400 400 T 800 400" fill="none" stroke="#1e40af" strokeWidth="80" opacity="0.2"/>

        {/* TOPレーン */}
        <path d="M 100 700 L 150 650 L 200 550 L 250 450 L 350 350 L 450 250 L 550 150 L 650 100 L 700 100" fill="none" stroke="#4b5563" strokeWidth="60" opacity="0.5"/>

        {/* MIDレーン */}
        <path d="M 100 700 L 200 600 L 300 500 L 400 400 L 500 300 L 600 200 L 700 100" fill="none" stroke="#4b5563" strokeWidth="60" opacity="0.5"/>

        {/* BOTレーン */}
        <path d="M 100 700 L 200 700 L 300 650 L 400 600 L 500 500 L 600 350 L 650 250 L 700 200 L 700 100" fill="none" stroke="#4b5563" strokeWidth="60" opacity="0.5"/>
        ...
        {Object.entries(areas).map(([key, area]) => {
            ...
            return (<g key={key}>
              {/* エリアの円 */}
              <circle cx={area.x} cy={area.y} r={isNexus ? 30 : isTower ? 20 : 25} fill={isSelected ? '#fbbf24' : key.includes('blue') ? '#3b82f6' : key.includes('red') ? '#ef4444' : '#6b7280'} fillOpacity={isNexus ? 0.8 : 0.4} stroke={isSelected ? '#fbbf24' : key.includes('blue') ? '#60a5fa' : key.includes('red') ? '#f87171' : '#9ca3af'} strokeWidth={isSelected ? 4 : 2} className="cursor-pointer hover:opacity-80 transition-opacity" onClick={() => handleAreaClick(key)}/>

              {/* エリアラベル */}
              <text x={area.x} y={area.y} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize={isNexus ? 14 : 10} fontWeight="bold" className="pointer-events-none">
                {area.label}
              </text>

              {/* キャラクター数の表示 */}
              {charsInArea.length > 0 && (<g>
                  <circle cx={area.x + 15} cy={area.y - 15} r={10} fill="#10b981" stroke="white" strokeWidth={2}/>
                  <text x={area.x + 15} y={area.y - 15} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize={10} fontWeight="bold" className="pointer-events-none">
                    {charsInArea.length}
                  </text>
                </g>)}
            </g>);
        ...
        {towers.filter(t => !t.destroyed).map((tower) => {
            ...
            if (!area)
                return null;
            return (<g key={tower.id}>
              <polygon points={`${area.x},${area.y - 15} ${area.x - 10},${area.y + 5} ${area.x + 10},${area.y + 5}`} fill={tower.team === 'blue' ? '#3b82f6' : '#ef4444'} stroke="white" strokeWidth={1}/>
            </g>);
        })}

        {/* キャラクターアイコン（簡易版） */}
        {characters.map((char) => {
            const area = areas[char.position.area];
            if (!area)
                return null;
            ...
            return (<g key={char.id}>
              <circle cx={area.x + offsetX} cy={area.y + offsetY + 30} r={8} fill={char.team === 'blue' ? '#60a5fa' : '#f87171'} stroke="white" strokeWidth={2} className="cursor-pointer"/>
              <text x={area.x + offsetX} y={area.y + offsetY + 30} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize={8} fontWeight="bold" className="pointer-events-none">
                {char.name.charAt(0)}
              </text>
            </g>);
        })}
