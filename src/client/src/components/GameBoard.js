      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-blue-900 bg-opacity-20 p-6 rounded-lg border-2 border-blue-500">
          ...
          <div className="space-y-3">
            ...
            {blueCharacters.length > 0 ? (blueCharacters.map((char) => (<CharacterCard key={char.id} character={char}/>))) : (<p className="text-gray-400">キャラクターがいません</p>)}
          </div>
        </div>
        ...
        <div className="bg-red-900 bg-opacity-20 p-6 rounded-lg border-2 border-red-500">
          ...
          <div className="space-y-3">
            <h4 className="text-lg font-semibold">キャラクター</h4>
            {redCharacters.length > 0 ? (redCharacters.map((char) => (<CharacterCard key={char.id} character={char}/>))) : (<p className="text-gray-400">キャラクターがいません</p>)}
          </div>
        </div>
      </div>
    </div>);
